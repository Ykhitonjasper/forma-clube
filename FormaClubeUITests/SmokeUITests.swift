...
/// Reviewer walkthrough: opens the app the way a first-time user does and captures the
...
///
/// The runner uninstalls first, so the app always starts
...
final class SmokeUITests: XCTestCase {
    ...
    private func completeOnboarding() {
        ...
        for _ in 0..<8 {
            ...
            // The launch capture already shows page one; grab the following pages only,
            // otherwise the first onboarding shot duplicates the launch shot pixel for pixel.
            if page > 0 { capture(named: "onboarding") }
            page += 1
            cta.tap()
            // A short settle can catch the last page mid-handoff and capture it twice.
            settle(1.0)
        }
